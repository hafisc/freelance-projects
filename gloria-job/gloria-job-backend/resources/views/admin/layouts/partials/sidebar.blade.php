...
<aside class="fixed inset-y-0 left-0 w-64 bg-primaryDark text-white z-50 transform md:translate-x-0 flex flex-col transition-transform duration-300 ease-in-out"
    ...
    <nav class="flex-grow py-6 px-4 space-y-1">
        ...
        <a href="{{ route('admin.companies.index') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all duration-200 {{ Route::is('admin.companies.*') ? 'bg-primaryBlue text-white shadow-lg shadow-primaryBlue/20' : 'text-white/75 hover:bg-white/5 hover:text-white' }}">
            <i class="fa-regular fa-building w-5 text-center text-lg"></i> Perusahaan
        </a>
        <a href="{{ route('admin.reports.index') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all duration-200 {{ Route::is('admin.reports.*') ? 'bg-primaryBlue text-white shadow-lg shadow-primaryBlue/20' : 'text-white/75 hover:bg-white/5 hover:text-white' }}">
            <i class="fa-solid fa-chart-pie w-5 text-center text-lg"></i> Laporan
        </a>
        <a href="{{ route('admin.system.index') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all duration-200 {{ Route::is('admin.system.*') ? 'bg-primaryBlue text-white shadow-lg shadow-primaryBlue/20' : 'text-white/75 hover:bg-white/5 hover:text-white' }}">
            <i class="fa-solid fa-gears w-5 text-center text-lg"></i> Sistem Berjalan
        </a>
    </nav>
