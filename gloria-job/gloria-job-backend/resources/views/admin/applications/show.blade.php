...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 md:p-8">
            ...
            <div class="space-y-6">
                ...
                <!-- CV Snapshot (saat melamar) -->
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-2">
                    <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">CV Saat Melamar</span>
                    <div class="sm:col-span-3">
                        @if($application->cv_path)
                            <a href="{{ route('admin.applications.cv', $application->id) }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2.5 bg-rose-50 hover:bg-rose-100 text-danger border border-rose-100 rounded-xl text-xs font-bold transition-all duration-150">
                                <i class="fa-solid fa-file-pdf text-sm"></i> Lihat / Unduh CV (Snapshot Saat Melamar)
                            </a>
                            <div class="text-xs text-slate-400 mt-1 flex items-center gap-1">
                                <i class="fa-solid fa-circle-info"></i> CV ini tersimpan otomatis saat pelamar mengirim lamaran.
                            </div>
                            @if($application->cv_shared_at)
                                <div class="text-xs text-emerald-500 mt-1 flex items-center gap-1 font-semibold">
                                    <i class="fa-solid fa-circle-check"></i> Sudah di-share ke perusahaan pada {{ \Carbon\Carbon::parse($application->cv_shared_at)->format('d M Y, H:i') }} WIB
                                </div>
                            @endif
                        @elseif($application->user && $application->user->cv)
                            <a href="{{ route('admin.applications.cv', $application->id) }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-600 border border-slate-200 rounded-xl text-xs font-bold transition-all duration-150">
                                <i class="fa-solid fa-file text-sm"></i> Lihat CV (dari Profil User)
                            </a>
                            <div class="text-xs text-amber-500 mt-1 flex items-center gap-1">
                                <i class="fa-solid fa-triangle-exclamation"></i> CV ini dari profil saat ini — bisa berbeda dengan saat melamar.
                            </div>
                        @else
            ...
            @if($application->job)
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-4 gap-2">
                        ...
                        <div class="sm:col-span-3">
                            <span class="text-sm font-bold text-slate-800">{{ $application->job->company_display_name }}</span>
                            @if($application->job->company)
                                <span class="text-xs text-slate-400 ml-2">— {{ $application->job->company->industry ?? '' }}</span>
                                <div class="mt-1">
                                    <a href="{{ route('admin.companies.show', $application->job->company->id) }}" class="text-xs text-primaryBlue font-semibold hover:underline">
                                        <i class="fa-solid fa-arrow-up-right-from-square mr-1"></i>Lihat Profil Perusahaan
                                    </a>
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-4 gap-2">
                        ...
                        <span class="sm:col-span-3 text-sm font-semibold text-slate-600">{{ $application->job->category ?? '-' }}</span>
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-4 gap-2">
                        <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">Gaji</span>
                        <span class="sm:col-span-3 text-sm font-semibold text-emerald-600">
                            {{ $application->job->salary_display ?? 'Tidak dicantumkan' }}
                        </span>
                    </div>
    ...
    <!-- Kanan: Update Status Form + Share -->
    <div class="space-y-6">
        <!-- Share ke Perusahaan -->
        @if($application->job && $application->job->company)
            <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
                <h5 class="text-sm font-bold text-slate-700 mb-4 flex items-center gap-2">
                    <i class="fa-solid fa-share-nodes text-emerald-500"></i> Share ke Perusahaan
                </h5>
                <p class="text-xs text-slate-500 mb-3">Tandai data pelamar ini sebagai sudah di-forward ke <strong>{{ $application->job->company->name }}</strong>.</p>
                <form action="{{ route('admin.applications.share', $application->id) }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 border border-emerald-200 font-bold rounded-xl text-sm transition-all">
                        <i class="fa-solid fa-share-nodes"></i>
                        {{ $application->cv_shared_at ? 'Share Ulang ke Perusahaan' : 'Share ke Perusahaan' }}
                    </button>
                </form>
            </div>
        @endif
