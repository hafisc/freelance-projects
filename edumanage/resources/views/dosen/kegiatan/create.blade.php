...
<div class="content-header">
    <div class="content-title">Isi Jurnal Mengajar Baru</div>
...
<div class="card" style="max-width: 700px; margin: 0 auto;">
    <div class="card-header">
        <div class="card-title">Form Input Kegiatan Mengajar (Dosen: {{ $dosen->nama }})</div>
    </div>
    
    <div class="card-body">
        <form action="{{ route('dosen.kegiatan.store') }}" method="POST">
            @csrf
            ...
            <div class="form-group">
                ...
                <input type="text" name="materi" id="materi" class="form-control @error('materi') is-invalid @enderror" placeholder="Materi yang diajarkan pada pertemuan ini..." value="{{ old('materi') }}" required>
                @error('materi')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                <div class="form-group">
                    ...
                    <input type="text" name="metode_pembelajaran" id="metode_pembelajaran" class="form-control @error('metode_pembelajaran') is-invalid @enderror" placeholder="Contoh: Ceramah dan Studi Kasus" value="{{ old('metode_pembelajaran') }}" required>
            ...
            <div class="form-group">
                ...
                <textarea name="tugas" id="tugas" class="form-control @error('tugas') is-invalid @enderror" placeholder="Isi detail deskripsi tugas mahasiswa..." rows="3">{{ old('tugas') }}</textarea>
                @error('tugas')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="catatan" id="catatan" class="form-control @error('catatan') is-invalid @enderror" placeholder="Catatan kelas, keaktifan mahasiswa, atau kendala..." rows="3">{{ old('catatan') }}</textarea>
                @error('catatan')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
