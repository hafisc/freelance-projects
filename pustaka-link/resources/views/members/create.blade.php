...
@section('page_title', 'Registrasi Anggota Baru')
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between bg-white p-6 rounded-lg border border-libborder shadow-sm">
        <div>
            <h2 class="text-base font-bold text-libnavy">Tambah Anggota</h2>
            <p class="text-xs text-libmuted mt-1">Daftarkan pembaca baru ke dalam sistem perpustakaan.</p>
        </div>
        ...
        <form action="{{ route('members.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="member_code" id="member_code" value="{{ old('member_code', $nextCode) }}" required
            ...
            <div>
                ...
                <textarea name="address" id="address" rows="3"
                    ...
                    placeholder="Alamat lengkap anggota perpustakaan...">{{ old('address') }}</textarea>
                @error('address')
                    <p class="text-xs text-libdanger mt-1.5 font-medium">{{ $message }}</p>
                @enderror
            </div>
