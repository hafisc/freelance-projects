...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
    <div>
        ...
        <p class="text-slate-400 text-xs mt-1">Daftar unggahan file materi kuliah, tugas, absensi, serta referensi kegiatan proses belajar.</p>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="border-b border-slate-100 text-slate-400 text-[11px] uppercase tracking-wider font-semibold">
                    ...
                    <th class="py-4 px-6">Batas Pengumpulan</th>
                    @if(session('role') === 'Admin' || session('role') === 'Dosen')
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="py-4 px-6 text-slate-500">
                            @if($k->jenis === 'Tugas' && $k->deadline)
                                <span class="text-red-500 font-semibold">{{ date('d-m-Y H:i', strtotime($k->deadline)) }} WIB</span>
                            @else
                        ...
                        @if(session('role') === 'Admin' || session('role') === 'Dosen')
                            <td class="py-4 px-6 text-right">
                                <div class="inline-flex items-center gap-2">
                                    ...
                                    <form action="{{ route('kegiatan.destroy', $k->id) }}" method="POST" class="m-0 inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kegiatan ini?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" 
        ...
        <div class="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl relative border border-slate-100 mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form action="{{ route('kegiatan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="judul" class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:outline-none focus:border-[#204a82] focus:ring-4 focus:ring-blue-100 transition-all duration-200" placeholder="Contoh: Modul Praktikum Basis Data" required>
                </div>
