...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
    <div>
        ...
        <p class="text-slate-400 text-xs mt-1">Kelola dan lihat rincian rencana pelaksanaan pertemuan proses belajar.</p>
    </div>
            ...
            <form action="{{ route('jadwal.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="kelas_id" class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:outline-none focus:border-[#204a82] focus:ring-4 focus:ring-blue-100 transition-all duration-200" required>
                        ...
                        @foreach($kelas as $k)
                            <option value="{{ $k->id }}">{{ $k->nama_kelas }} (Kode: {{ $k->kode_kelas }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="pertemuan_ke" class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:outline-none focus:border-[#204a82] focus:ring-4 focus:ring-blue-100 transition-all duration-200" placeholder="Contoh: 1" required>
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <input type="text" name="ruangan" class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:outline-none focus:border-[#204a82] focus:ring-4 focus:ring-blue-100 transition-all duration-200" placeholder="Contoh: Lab 1 atau R.302" required>
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Topik / Bahasan Materi</label>
                    <input type="text" name="topik_materi" class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:outline-none focus:border-[#204a82] focus:ring-4 focus:ring-blue-100 transition-all duration-200" placeholder="Contoh: Pengenalan OOP">
                </div>
                ...
                <div>
                    ...
                    <textarea name="catatan" class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:outline-none focus:border-[#204a82] focus:ring-4 focus:ring-blue-100 transition-all duration-200" rows="2" placeholder="Catatan tambahan hasil kelas..."></textarea>
                </div>
            ...
            <div class="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl relative border border-slate-100 mx-4 max-h-[90vh] overflow-y-auto">
                ...
                <form action="{{ route('jadwal.update', $j->id) }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="kelas_id" class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:outline-none focus:border-[#204a82] focus:ring-4 focus:ring-blue-100 transition-all duration-200" required>
                            @foreach($kelas as $k)
                                <option value="{{ $k->id }}" {{ $j->kelas_id == $k->id ? 'selected' : '' }}>{{ $k->nama_kelas }} (Kode: {{ $k->kode_kelas }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
